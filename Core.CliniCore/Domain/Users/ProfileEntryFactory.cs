...
namespace Core.CliniCore.Domain.Users
{
    public static class ProfileEntryFactory
    {
            ...
            IValidator<T>? validator = null, bool isRequired = false)
        {
            ...
            // Pass error message provider for dynamic error messages (CompositeValidator)
            Func<string>? errorMessageProvider = validator != null ? () => validator.ErrorMessage : null;
            return new ProfileEntry<T>(key, displayName, isRequired, validatorFunc,
                errorMessage: null, errorMessageProvider: errorMessageProvider);
        }
